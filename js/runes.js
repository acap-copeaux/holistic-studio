/* ============================================================
   ...
   Elder Futhark complet, puriste + psycho + analyste
   Tirages : 1 / 3 / 5 / 9 runes
   Premium : tirages étendus + axe runique personnel
   ============================================================ */

import { createSeed, mulberry32 } from "./utils.js";
import { isPremium, isLifetime, getAccessLevel } from "./premium.js";

/* ------------------------------------------------------------
   0. Helper DOM local (pour ce module)
   ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   1. Base de données Elder Futhark — 24 runes
   ------------------------------------------------------------ */
...
const ELDER_FUTHARK = [
  ...
  },
  {
    ...
    analyst:
      ...
      "Une information, une lucidité ou une compréhension nouvelle peut transformer ta manière de agir.",
    reversedHint:
...
/* ------------------------------------------------------------
   2. Position dans le tirage
   ------------------------------------------------------------ */

function runePositionLabel(count, index) {
  if (count === 1) {
    return "Message du moment";
  }
  if (count === 3) {
...
/* ------------------------------------------------------------
   3. Tirage aléatoire déterministe
   ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   4. Axe runique personnel (tirage 9 runes, Lifetime)
   ------------------------------------------------------------ */

function computeRuneAxis(runes) {
  const themes = {
    "corps_et_vitalite": ["uruz", "eihwaz", "isa"],
    "richesse_et_flu x": ["fehu", "jera", "hagalaz"],
  ...
  if (mainAxis && mainAxis[1] > 0) {
    ...
    txt += `<p>Cette série de runes montre que ton chemin actuel se concentre surtout sur ce domaine : ` +
  ...
  if (!txt) {
    txt = `<p>Aucun axe dominant ne ressort clairement dans ce tirage. C’est une indication que ta dynamique actuelle ` +
          `se joue plutôt dans un équilibre très large, sans suraccent sur un seul terrain.</p>`;
  }

  txt += `<p class="subnote">Cet axe runique personnel est une carte symbolique. Il ne fixe pas ton destin, ` +
         `mais propose un fil rouge à observer dans les mois à venir.</p>`;

  return txt;
}

/* ------------------------------------------------------------
   5. Construction du texte d’interprétation
   ------------------------------------------------------------ */

function renderRunesReading(root, runes, count) {
  const resultEl = q(root, "#runes-output");
  if (!resultEl) return;
  ...
  html += `<p class="subnote">Tirage : <strong>${count} rune(s)</strong> — Niveau d’accès : <strong>${lvl.toUpperCase()}</strong></p>`;
...
/* ------------------------------------------------------------
   6. Garde-fou d’accès (FREE vs Premium)
   ------------------------------------------------------------ */

function guardAccess(root, count) {
  const lvl = getAccessLevel();
  const premium = isPremium();
  const out = q(root, "#runes-output");
  if (!out) return false;
...
/* ------------------------------------------------------------
   7. Tirage complet (logique principale)
   ------------------------------------------------------------ */

function runesDoDraw(root, count) {
  if (!guardAccess(root, count)) return;

  const firstnameEl = document.getElementById("a-firstname");
  const dateEl = document.getElementById("a-date");
  const placeEl = document.getElementById("a-place");
  const qEl = q(root, "#runes-question");
  ...
  const seedSource =
    `${firstname}|${date}|${place}|${question}|${count}|${today}`;
...
/* ------------------------------------------------------------
   8. Texte d’aide (infos d’accès)
   ------------------------------------------------------------ */

function updateRunesAccessHint(root) {
  const hint = q(root, "#runes-access-hint");
  if (!hint) return;
  const lvl = getAccessLevel();
  ...
  if (lvl === "free") {
    ...
    hint.innerHTML =
      'Niveau actuel : <strong>ACCÈS À VIE</strong> — tirages 1 / 3 / 5 / 9 runes ' +
      'complets + axe runique personnel sur les 9 runes.';
  }
}

/* ------------------------------------------------------------
   9. Initialisation du module (appelé par app.js OU auto)
   ------------------------------------------------------------ */

export function initModule(rootElement) {
  const root = rootElement || document;

  const b1 = q(root, "#btn-runes-1");
  const b3 = q(root, "#btn-runes-3");
  const b5 = q(root, "#btn-runes-5");
  const b9 = q(root, "#btn-runes-9");
  ...
  const questionInput = q(root, "#runes-question");
  if (questionInput) {
...
/* ------------------------------------------------------------
   10. Auto-init de secours (si app.js ne rappelle pas initModule)
   ------------------------------------------------------------ */

(function autoInitFallback() {
  // On tente de s’initialiser quelques fois si app.js ne le fait pas
  let tries = 0;
  const maxTries = 10;

  function tryInit() {
    const container = document.getElementById("module-container") || document;
    const btn = container.querySelector("#btn-runes-1");
    if (btn) {
